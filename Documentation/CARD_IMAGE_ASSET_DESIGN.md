...
> **STATUS UPDATE (Step 71, current):** OD-1 (below) is **resolved**. The product owner has determined that Spread Review must display actual rendered card artwork — the existing text/tile presentation is confirmed as an MVP-era placeholder, not the final intended presentation. See `Documentation/STEP71_OD1_PRODUCT_DECISION.md` for the authoritative decision record. **OD-2 (artwork source) and OD-3 (serving architecture) remain open** — this update resolves only whether artwork is required, not which artwork or how it is served. Card Entry's own separate "visual card browsing" deferral (Section 4/9 below) is unaffected. Everything else in this document below is preserved as the original Step 48 investigation and remains accurate background/history except where a specific passage is marked as superseded by this update.
...
**Genuinely unresolved as of this step (Step 48); resolved by Step 72 — see below:**
...
**[Step 72 update]:** OD-2 has since been resolved. Individual-file-level verification (not a category-level blanket claim) was performed against the Wikimedia Commons "Rider-Waite-Smith tarot deck (TaionWC)" 78-file set, identified as the "Pam-A" 1910 Rider trade-edition scan. See `Documentation/STEP72_OD2_ARTWORK_SOURCE_INVESTIGATION.md` for the full evidence, the candidate comparison against Wikimedia's "Geldard" set and an Internet Archive candidate, and the stated caveats/limitations of this verification. No image file has been downloaded or added to this repository as a result.
...
- Fully resolves Card Entry (which the spec does not require images for) permanently. For Spread Review, **[Step 71 update]: OD-1 is now resolved — a non-image placeholder is not the final intended presentation there.** Option E remains the correct description of Spread Review's *current, interim* implementation (unchanged since MVP) but is no longer a candidate for Spread Review's *final* state; it stays fully valid as Card Entry's permanent approach and as Spread Review's placeholder pending OD-2/OD-3.
...
- Whether Spread Review's "full visual layout... cards visible" requires artwork rendering, or is satisfied by a text/name-based layout (per Option E), was Open Product Decision OD-1 (Section 11) — **not resolved by this step at the time it was written**, and not something the existing API contract needs to change to support either answer, since `image_ref` (present or null) flows through either way. **[Step 71 update]: OD-1 is now resolved — artwork rendering is required. See `Documentation/STEP71_OD1_PRODUCT_DECISION.md`.**
- No change to `GET /readings/{reading_id}` is required by OD-1's resolution — `image_ref` already flows through this contract unchanged; only the frontend's *rendering* of it is affected, and that is explicitly not implemented by this update (OD-2/OD-3 remain open, Section 11).
...
- **OD-1 — RESOLVED (Step 71).** Does the Product Spec's Spread Review requirement ("full visual layout of all positions, cards, and orientations," "all cards visible") require rendered card artwork, or is it satisfiable by a well-designed non-image layout (card name + orientation, per Option E)? The spec's own text never used the words "image" or "artwork" for this screen, which is why this was originally left open. **Resolved by explicit product-owner decision (Step 71): yes, Spread Review must display actual rendered card artwork.** The text/tile presentation shipped for MVP is confirmed as a temporary placeholder, not the final intended presentation. See `Documentation/STEP71_OD1_PRODUCT_DECISION.md` for the full decision record. This resolution does not select an artwork source or serving architecture — OD-2 and OD-3 below remain fully open.
- **OD-2 — RESOLVED (Step 72).** Which specific artwork source/file set will actually be used, and has its specific license been verified at the file level (Section 5)? **Resolved: the Wikimedia Commons "Rider-Waite-Smith tarot deck (TaionWC)" category (78 files, the "Pam-A" 1910 Rider trade-edition scan) is the approved candidate source**, verified at the individual-file level (not merely the category blurb) against the repository's existing licensing policy. See `Documentation/STEP72_OD2_ARTWORK_SOURCE_INVESTIGATION.md` for the full evidence, candidate comparison, and stated caveats. **This does not mean artwork has been added** — no image file has been downloaded or committed, and OD-3 (serving architecture) remains fully open below.
...
- **Whether card artwork was mandatory for MVP launch itself** — resolved by history, not by this step: Raidian Wise's MVP (`v0.2.0-mvp`) already shipped with Option E's text/tile placeholder on both Card Entry and Spread Review, so this specific question is moot. **[Step 71 update]: OD-1's resolution is about the final intended presentation, not the already-shipped MVP** — the current text/tile placeholder remains an acceptable interim implementation on Spread Review until real artwork (OD-2/OD-3) is built; nothing about the already-released MVP baseline is retroactively invalidated by OD-1's resolution.
